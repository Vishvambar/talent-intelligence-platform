...
│       ├── bm25.pkl
│       ├── top_3000.parquet
│       ├── recall_audit.json                  ← Phase 6.5 output
│       ├── synthetic_labels.parquet
│       ├── label_quality_audit.json           ← Phase 7.15 output
...
│       ├── xgb_ranker.pkl
│       ├── reasoning.parquet
│       ├── reasoning_diversity_audit.json     ← Phase 10.5 output
│       ├── runtime_report.md
...
│   ├── phase03_7_behavioral_intelligence.py
│   ├── phase04_integrity.py
│   ├── phase05_retrieval_infra.py
│   ├── phase06_hybrid_retrieval.py
│   ├── phase06_5_recall_audit.py            ← NEW
│   ├── phase07a_teacher_ensemble.py
│   ├── phase07_15_label_quality_audit.py    ← NEW
│   ├── phase07b_label_calibration.py
│   ├── phase07c_pairwise.py
│   ├── phase07_25_feature_parity_audit.py
│   ├── phase08_ltr.py
│   ├── phase08_5_shap.py
│   ├── phase08_75_stress_test.py
│   ├── phase09_ensemble.py
│   ├── phase09_5_elite_rerank.py
│   ├── phase10_reasoning_bank.py
│   ├── phase10_5_reasoning_diversity.py     ← NEW
│   ├── phase11_runtime_verify.py
│   └── phase11_5_submission_audit.py
├── online/
...
> **One rule that will save you days:** Never tune Tier 2 before Tier 1 stabilizes.

Every tunable value in this system belongs to exactly one tier. Tier determines the order in which you touch them. If you find yourself adjusting `num_leaves` before you've validated teacher weights, you are optimizing noise.

### Tier 1 — Highest Impact: Tune First

These parameters determine what signal enters the model. Getting them wrong means the model trains on the wrong objective regardless of how well the model is tuned.

**Retrieval Weights** (`dense_weight`, `bm25_weight`, `career_weight`, `ontology_weight`)

Changing these changes which 3,000 candidates the model sees. A misweighted retrieval stage means truly relevant candidates never enter the pipeline. Tune using Recall@3000 from Phase 6.5. Target: Recall@3000 > 0.90. Order of tuning: start with `0.35 / 0.25 / 0.25 / 0.15`, adjust in increments of 0.05.

**Teacher Weights** (`technical_weight`, `evaluation_weight`, `execution_weight`)

These determine what the synthetic labels encode. `evaluation_weight` (Teacher B: NDCG/MAP) should be high — it is the rarest signal and the one most uniquely aligned with the JD. If SHAP shows `evaluation_score` is not in the top 5 features, raise `evaluation_weight` and regenerate.

Current: `0.45 / 0.35 / 0.20`. Do not touch these until you have run Phase 8.5 SHAP once.

### Tier 2 — Medium Impact: Tune After Tier 1 Is Stable

These parameters control how well the model learns from the signal. They matter, but only once the signal itself is clean.

**LTR Hyperparameters** (`num_leaves`, `learning_rate`, `max_depth`, `n_estimators`)

The starting values (`63 / 0.05 / 8 / 500 with early stopping`) are sensible defaults for a 3,000-sample dataset with ~30 features. Tune these only after:
- Phase 6.5 Recall Audit is green
- Phase 7.15 Label Quality Audit is green
- Phase 8.5 SHAP shows the right features in the top 15

Use the Phase 8.75 Stress Test to validate any Tier 2 change. If the Composite score doesn't improve by ≥ 0.002, revert.

**Ensemble Weights** (`lgbm_weight`, `xgb_weight`)

Start at `0.6 / 0.4`. Adjust only if Phase 8.75 shows XGBoost has meaningfully different and better top-10 ordering than LGBM.

### Tier 3 — Low Impact: Touch Last

These are regularization parameters. They rarely make a meaningful difference in a 3,000-sample dataset. Touch only if you observe overfitting (large gap between training and validation metric in Phase 8.75).

```yaml
# LightGBM regularization — do not tune unless Tier 1 and Tier 2 are locked
...
### The Single Tuning Protocol

For any proposed change to any hyperparameter:

```
1. Make the change in config/hyperparams.yaml (never in the code directly)
2. Re-run only the phases downstream of the change:
   - Tier 1 retrieval change → re-run from Phase 6
   - Tier 1 teacher change  → re-run from Phase 7A
   - Tier 2 LTR change      → re-run from Phase 8
3. Re-run Phase 8.75 Stress Test
4. Compare Composite score to the previous version
5. If improvement >= 0.002: commit the change, update the version log
6. If improvement < 0.002: revert immediately
7. Never make two changes at once — you cannot isolate the cause of improvement
```
...
PHASE 3.5  Career Intelligence Layer          ← added: review round 1
...
PHASE 6    Hybrid Retrieval
          ...
          TOP 3000 CANDIDATES  [top_k: LOCKED]
               ↓
PHASE 6.5  Retrieval Recall Audit            ← added: review round 3 [NEW]
               ↓ (must pass before spending LLM budget)
PHASE 7A   Teacher Ensemble
PHASE 7.15 Label Quality Audit              ← added: review round 3 [NEW]
PHASE 7B   Label Calibration
PHASE 7C   Pairwise Refinement
PHASE 7.25 Feature Parity Audit             ← added: review round 2
               ↓
PHASE 8    Learning To Rank  [Huber primary · LambdaRank experiment only]
PHASE 8.5  SHAP Feedback Loop
PHASE 8.75 Retrieval & Ranking Stress Test  ← added: review round 2
PHASE 9    Ensemble Models
PHASE 9.5  Elite Re-Ranking                 ← added: review round 1
               ↓
PHASE 10   Reasoning Bank
PHASE 10.5 Reasoning Diversity Audit       ← added: review round 3 [NEW]
PHASE 11   Runtime Verification
PHASE 11.5 Submission Safety Audit          ← added: review round 2
               ↓
